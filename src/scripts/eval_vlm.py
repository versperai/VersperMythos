...
from src.vlm.model_vlm import VLM, VLMConfig
from src.vlm.trainer.trainer_utils import setup_seed, get_model_params
warnings.filterwarnings('ignore')

def init_model(args):
    tokenizer = AutoTokenizer.from_pretrained(args.load_from, trust_remote_code=True)
    ckp_keywords = ['out/', 'checkpoints/', '.pth']
    if not any(k in args.load_from for k in ckp_keywords):
...
def main():
    ...
    parser.add_argument('--load_from', default='model/tokenizer', type=str, help="模型加载路径（tokenizer路径，或HF格式模型目录）")
    parser.add_argument('--save_dir', default='out/vlm', type=str, help="模型权重目录")
    ...
    parser.add_argument('--num_key_value_heads', default=4, type=int, help="KV头数")
    parser.add_argument('--intermediate_size', default=None, type=int, help="FFN中间层维度（默认根据hidden_size自动计算）")

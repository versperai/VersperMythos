"""
MiniMind-VLM — Vision-Language Model.

Extends MiniMindForCausalLM with a SigLIP2 vision encoder and
MLP vision-language projector.

Architecture: SigLIP2 → MLP Projector → MiniMind LLM Backbone
"""
...
class VLM(LMForCausalLM):
    ...
    def forward(self,
        ...
        if pixel_values is not None and start_pos == 0:
            if hasattr(pixel_values, 'keys'):
                ...
                vision_tensors = torch.stack([
                    self.vision_proj(
                        VLM.get_image_embeddings(pixel_values[:, i, :, :, :], self.vision_encoder)
                    ) for i in range(num)
                ], dim=1)
        ...
        aux_loss = aux_loss + sum(p.sum() for p in self.vision_proj.parameters()) * 0  # dummy for DDP
